...
type
  TWinCCOPCClient = class(TComponent)
  private
...
constructor TWinCCOPCClient.Create(AOwner : TComponent);
var
  i : byte;
begin
...
procedure TWinCCOPCClient.ReadTimerTimer(Sender: TObject);
begin
  Read;
end;

procedure TWinCCOPCClient.Read;
var
  i : byte;
...
procedure TWinCCOPCClient.ConnectTimerTimer(Sender: TObject);
begin
  Connect;
end;

procedure TWinCCOPCClient.Connect;
var
  i : byte;
...
begin
  ...
  if not Powersock1.BeenTimedOut then
    begin
      try
        WinCCServerIf := CreateComObject(ProgIDToClassID(WinCCServerProgID)) as IOPCServer;
      except
        WinCCServerIf := nil;
      end;
    end;
...
procedure TWinCCOPCClient.Disconnect;
begin
...
function TWinCCOPCClient.GetRecommend(Index : byte) : single;
begin
  Result := FRecommend[Index];
end;

procedure TWinCCOPCClient.SetRecommend(Index : byte; Value : single);
var
  HR : HResult;         //Результат OLE-операции
begin
...
procedure TWinCCOPCClient.SetEnable(Index : byte; Value : boolean);
begin
  FEnable[Index] := Value;
end;

procedure TWinCCOPCClient.SetCalibration(Value : boolean);
var
  HR : HResult;         //Результат OLE-операции
begin
  FCalibration := Value;
...
function TWinCCOPCClient.GetAssigned(Index : byte) : single;
begin
  Result := FAssigned[Index];
end;

function TWinCCOPCClient.GetConv1(Index : byte) : boolean;
begin
  Result := FConv1[Index];
end;

function TWinCCOPCClient.GetEnable(Index : byte) : boolean;
begin
  Result := FEnable[Index];
end;

procedure TWinCCOPCClient.SetConnected(Value : boolean);
begin
