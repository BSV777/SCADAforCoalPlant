...
type
  ...
  private
    ...
    QueryOracle : TQuery;                                    //Компонент TQuery
...
begin
  ...
  QueryOracle := TQuery.Create(Self);
  QueryOracle.SessionName := 'OracleSession';
  QueryOracle.DatabaseName := 'UPCTracing';
end;
...
begin
  ...
  QueryOracle.SQL.Clear;
  QueryOracle.SQL.Add('select * from COAL order by grade');
  QueryOracle.Open;
  FGradesCount := QueryOracle.RecordCount - 1;
  ...
  for i := 1 to FGradesCount do
    begin
      FGradeNames[i] := QueryOracle.Fields.FieldByName('name').AsString;
      QueryOracle.Next;
    end;
  QueryOracle.Close;
end;
...
begin
  QueryOracle.SQL.Clear;
  QueryOracle.SQL.Add('insert into COAL values(' + IntToStr(FGradesCount + 1) + ', ''' + Value + ''', '' '',0,0);');
  QueryOracle.ExecSQL;
  Query1.SQL.Clear;
