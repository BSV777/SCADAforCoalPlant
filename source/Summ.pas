...
begin
  ...
  if pStorage.DONum = 1 then
    QRTitle.Caption := 'поданных на угольные башни №5 и №6 за период:' else
  ...
  QRChart.Chart.LeftAxis.Maximum := 400;
  for ObjEnum := 1 to PrintingObjects.Count do
    begin
      ...
      SQLString := 'select DAT, PR_TH from grade2_' + IntToStr(FGrade.GradeNum) + ' where (dat > ''' +
