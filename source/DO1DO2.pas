...
type
  TDO1DO2Form = class(TForm)
    ...
    deBeginDate1: TDateEdit;
    deEndDate1: TDateEdit;
    ...
    procedure UpDown1MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure rgReportCategory1Click(Sender: TObject);
    procedure rgReportType1Click(Sender: TObject);
    procedure deReportDate1Change(Sender: TObject);
    procedure deBeginDate1Change(Sender: TObject);
    procedure deEndDate1Change(Sender: TObject);
    procedure btMakeTrends1Click(Sender: TObject);
    procedure btMakeReport1Click(Sender: TObject);
    procedure SheetAlarms1Show(Sender: TObject);
    procedure deAlarmsDate1Change(Sender: TObject);
    procedure clbAlarms1ClickCheck(Sender: TObject);
  private
    FGrade : TCoalGrade;                         //Экземпляр объекта TCoalGrade
    procedure MakeReport(DONum : byte; BeginTime, EndTime : TDateTime);
    procedure MakeMonthReport(DONum : byte);
    procedure MakeBatchReport(DONum : byte; BeginTime, EndTime : TDateTime);
  public
...
//          EndTime := deReportDate2.Date + StrToTime(meEndTime2.Text + ':59');
      ...
      if rgReportCategory2.ItemIndex = 0 then MakeBatchReport(2, BeginTime, EndTime) else MakeReport(2, BeginTime, EndTime);
    end;
  WStationForm.OPCReadTimer.Enabled := True;                             //Включить обновление данных
...
//        EndTime := deReportDate2.Date + StrToTime(meEndTime2.Text + ':00');
    ...
    if ViewingObjects.Count > 0 then ViewForm.ViewTrends(2, BeginTime, EndTime, ViewingObjects);  //Формирование графиков
    WStationForm.OPCReadTimer.Enabled := True;                             //Включить обновление данных
    Screen.Cursor := crDefault;                               //Включить песочные часы
  finally
    btMakeTrends2.Enabled := True;                             //Включить кнопку
...
procedure TDO1DO2Form.MakeReport(DONum : byte; BeginTime, EndTime : TDateTime);
begin
  SummForm := nil;
  try
    SummForm := TSummForm.Create(Self);                 //Создать форму
    if DONum = 1 then SummForm.Initialize(Storage1, BeginTime, EndTime) else SummForm.Initialize(Storage2, BeginTime, EndTime);
...
var
  myRect: TGridRect;
begin
  rgReportCategory1Click(Sender);
  rgReportType1Click(Sender);
...
begin
  if DONum = 1 then
    begin
      pStorage := @Storage1;
      if not CanRedraw1 then CurrHour1 := UpDown1.Position;
      DBChart1.LeftAxis.Minimum := 0;
      DBChart1.LeftAxis.Maximum := 200;
      UpDown1.Enabled := False;
      UpDown1.Position := CurrHour1;
      lbChTime1.Caption := IntToStr(CurrHour1) + ':00 - ' + IntToStr(CurrHour1) + ':59';
      BeginTime := FormatDateTime('dd/mm/yyyy', Now) + ' ' + IntToStr(CurrHour1) + ':00:00';
      EndTime := FormatDateTime('dd/mm/yyyy', Now) + ' ' + IntToStr(CurrHour1) + ':59:59';
      if DBChart1.BottomAxis.Minimum > StrToDateTime(EndTime) then
        begin
          DBChart1.BottomAxis.Minimum := StrToDateTime(BeginTime);
          DBChart1.BottomAxis.Maximum := StrToDateTime(EndTime);
        end else
        begin
          DBChart1.BottomAxis.Maximum := StrToDateTime(EndTime);
          DBChart1.BottomAxis.Minimum := StrToDateTime(BeginTime);
        end;
      DBChart1.BottomAxis.DateTimeFormat := 'hh:mm';
      DBChart1.BottomAxis.Increment := DateTimeStep[dtFiveMinutes];
      UpDown1.Enabled := True;
    end else
    begin
      pStorage := @Storage2;
      if not CanRedraw2 then CurrHour2 := UpDown2.Position;
      DBChart2.LeftAxis.Minimum := 0;
      DBChart2.LeftAxis.Maximum := 400;
      UpDown2.Enabled := False;
      UpDown2.Position := CurrHour2;
      lbChTime2.Caption := IntToStr(CurrHour2) + ':00 - ' + IntToStr(CurrHour2) + ':59';
      BeginTime := FormatDateTime('dd/mm/yyyy', Now) + ' ' + IntToStr(CurrHour2) + ':00:00';
      EndTime := FormatDateTime('dd/mm/yyyy', Now) + ' ' + IntToStr(CurrHour2) + ':59:59';
...
procedure TDO1DO2Form.UpDown1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  CanRedraw1 := False;
  RedrawTrends(1);
...
procedure TDO1DO2Form.rgReportCategory1Click(Sender: TObject);
var
  i : byte;
begin
  clbReportCategory1.Items.Clear;
  case rgReportCategory1.ItemIndex of
    0 : begin
          rbMonth1.Visible := False;
          if rgReportType1.ItemIndex = 0 then clbReportCategory1.Visible := False else clbReportCategory1.Visible := True;
          for i := 1 to 14 do
            begin
              Batch := BatchList1.Items[i - 1];
              clbReportCategory1.Items.Add('Дозатор №' + IntToStr(i) + ' - ' + Batch.GrName);
            end;
        end;
    1:  begin
          if rgReportType1.ItemIndex = 0 then rbMonth1.Visible := True else rbMonth1.Visible := False;
          clbReportCategory1.Visible := False;
        end;
  end;
end;

procedure TDO1DO2Form.rgReportType1Click(Sender: TObject);
begin
  case rgReportType1.ItemIndex of
    0 : begin
          clbReportCategory1.Visible := False;
          btMakeTrends1.Visible := False;
          btMakeReport1.Visible := True;
          if rgReportCategory1.ItemIndex = 0 then rbMonth1.Visible := False else rbMonth1.Visible := True;
        end;
    1:  begin
          if rgReportCategory1.ItemIndex = 0 then clbReportCategory1.Visible := True else clbReportCategory1.Visible := False;
          btMakeTrends1.Visible := True;
          btMakeReport1.Visible := False;
          rbMonth1.Visible := False;
        end;
  end;
end;

procedure TDO1DO2Form.deReportDate1Change(Sender: TObject);
begin
  rbMonth1.Caption := FormatDateTime('mmmm yyyy', deReportDate1.Date);
end;

procedure TDO1DO2Form.deBeginDate1Change(Sender: TObject);
begin
  rbMonth1.Caption := FormatDateTime('mmmm yyyy', deReportDate1.Date);
end;

procedure TDO1DO2Form.deEndDate1Change(Sender: TObject);
begin
  rbMonth1.Caption := FormatDateTime('mmmm yyyy', deReportDate1.Date);
end;

procedure TDO1DO2Form.btMakeTrends1Click(Sender: TObject);
var
  ObjEnum : byte;           //Перечислитель объектов
  ViewingObjects : TList;   //Перечень объектов для просмотра трендов
  BeginTime, EndTime : TDateTime;
begin
  ...
  try
    WStationForm.OPCReadTimer.Enabled := False;      //Отключить обновление данных
    btMakeTrends1.Enabled := False;      //Погасить кнопку
    btMakeReport1.Enabled := False;      //Погасить кнопку
    Screen.Cursor := crHourGlass;       //Включить песочные часы
    ViewForm := TViewForm.Create(Self); //Создать форму для просмотра графиков
    if rbDay1.Checked then
      begin
        BeginTime := deReportDate1.Date + StrToTime('08:00:00');
        EndTime := deReportDate1.Date + StrToTime('20:00:00');
      end else
    if rbNight1.Checked then
      begin
        BeginTime := deReportDate1.Date - 1 + StrToTime('20:00:00');
        EndTime := deReportDate1.Date + StrToTime('08:00:00');
      end else
      begin
//        BeginTime := deReportDate1.Date + StrToTime(meBeginTime1.Text + ':00');
//        EndTime := deReportDate1.Date + StrToTime(meEndTime1.Text + ':00');
        BeginTime := deBeginDate1.Date + StrToTime(meBeginTime1.Text + ':00');
        EndTime := deEndDate1.Date + StrToTime(meEndTime1.Text + ':00');
      end;
    case rgReportCategory1.ItemIndex of  //Выбор типа графиков
    0 : begin
          for ObjEnum := 1 to 14 do
            begin
              Batch := BatchList1.Items[ObjEnum - 1];
              if clbReportCategory1.State[ObjEnum - 1] = cbChecked then ViewingObjects.Add(Batch);
            end;
        end;
    1:  begin
          for ObjEnum := 1 to Storage1.GradesCount do
            begin
              if Storage1.GradeWasLoaded(ObjEnum, BeginTime, EndTime) then
                begin
                  FGrade := TCoalGrade.Create();
                  ViewingObjects.Add(FGrade);
                  FGrade.GradeNum := ObjEnum;
                  FGrade.GradeName := Storage1.GradeNames[ObjEnum];
                end;
            end;
        end;
    end;
    if ViewingObjects.Count > 0 then ViewForm.ViewTrends(1, BeginTime, EndTime, ViewingObjects);  //Формирование графиков
    WStationForm.OPCReadTimer.Enabled := True;                             //Включить обновление данных
    Screen.Cursor := crDefault;                               //Включить песочные часы
  finally
    btMakeTrends1.Enabled := True;                             //Включить кнопку
    btMakeReport1.Enabled := True;                             //Включить кнопку
    if Assigned(ViewForm) then ViewForm.Release;              //Удалить форму с графиками
  end;
  ViewingObjects.Free;                                      //Удаление списка объектов
end;

procedure TDO1DO2Form.btMakeReport1Click(Sender: TObject);
var
  BeginTime, EndTime : TDateTime;
begin
  WStationForm.OPCReadTimer.Enabled := False;      //Отключить обновление данных
  ...
  if rbMonth1.Checked then
    ...
    begin
      ...
      if rbNight1.Checked then
        ...
        begin
//          BeginTime := deReportDate1.Date + StrToTime(meBeginTime1.Text + ':00');
//          EndTime := deReportDate1.Date + StrToTime(meEndTime1.Text + ':59');
          BeginTime := deBeginDate1.Date + StrToTime(meBeginTime1.Text + ':00');
          EndTime := deEndDate1.Date + StrToTime(meEndTime1.Text + ':59');
        end;
      if rgReportCategory1.ItemIndex = 0 then MakeBatchReport(1, BeginTime, EndTime) else MakeReport(1, BeginTime, EndTime);
    end;
  WStationForm.OPCReadTimer.Enabled := True;                             //Включить обновление данных
  btMakeTrends1.Enabled := True;                             //Включить кнопку
  btMakeReport1.Enabled := True;                             //Включить кнопку
...
procedure TDO1DO2Form.SheetAlarms1Show(Sender: TObject);
var
  CurrRaw, CurrLine, j : integer;
  PriorState, PriorWrk, CurrState, CurrWrk : byte;
  NewMessage : boolean;
...
begin
  ...
  DO1Form.DeSelectAll;
  DO2Form.DeSelectAll;
  BeginTime := FormatDateTime('yyyy/mm/dd', deAlarmsDate1.Date) + ' 00:00:00';
  EndTime := FormatDateTime('yyyy/mm/dd', deAlarmsDate1.Date) + ' 23:59:59';
  AlarmsQuery := TQuery.Create(Self);
  AlarmsQuery.SessionName := 'MySession';
  AlarmsQuery.DatabaseName := 'UPC';
  AlarmsQuery.SQL.Clear;
  AlarmsQuery.SQL.Add('select * from CONFIG1 where (dat > ''' + BeginTime + ''') and (dat < ''' + EndTime + ''') order by dat;');
  AlarmsQuery.Open;
  AlarmsQuery.Last;
  SendMessage(sgAlarms1.Handle, WM_SETREDRAW, 0, 0);

  sgAlarms1.RowCount := 2;
  sgAlarms1.Cells[0, 1] := '';
  sgAlarms1.Cells[1, 1] := '';
  sgAlarms1.Cells[2, 1] := '';
  CurrRaw := 1;
  ...
  AssignFile(F, 'Messages1.txt');
  Rewrite(F);
  Writeln(F, 'Журнал событий за: ' + FormatDateTime('dd/mm/yyyy', deAlarmsDate1.Date));
  ...
  for CurrLine := AlarmsQuery.RecordCount - 1 downto 1 do
    begin
      ...
      for j := 1 to 14 do
        begin
          if clbAlarms1.State[i] = cbChecked then
            begin
              CurrState :=AlarmsQuery.Fields.FieldByName('state_' + IntToStr(j)).AsInteger;
              CurrWrk := AlarmsQuery.Fields.FieldByName('wrk_' + IntToStr(j)).AsInteger;
              AlarmsQuery.Prior;
              ...
              PriorWrk := AlarmsQuery.Fields.FieldByName('wrk_' + IntToStr(j)).AsInteger;
              AlarmsQuery.Next;
              sgAlarms1.Cells[2, CurrRaw] := '';
              if CurrState <> PriorState then
                begin
                  NewMessage := True;
                  sgAlarms1.Cells[1, CurrRaw] := '   ' + IntToStr(j);
                  case CurrState of
                  0: sgAlarms1.Cells[2, CurrRaw] := ' +   OK.';
                  1: sgAlarms1.Cells[2, CurrRaw] := ' !   Нарушение работы тензодатчика.';
                  2: sgAlarms1.Cells[2, CurrRaw] := '!!!  Потеряна связь с "Микросимом".';
                  3: sgAlarms1.Cells[2, CurrRaw] := ' -   Дозатор выведен из работы.';
                  end;
                end;
              if (CurrState = 0) and (CurrWrk <> PriorWrk) then
                begin
                  ...
                  sgAlarms1.Cells[1, CurrRaw] := '   ' + IntToStr(j);
                  case CurrWrk of
                  0: sgAlarms1.Cells[2, CurrRaw] := sgAlarms1.Cells[2, CurrRaw] + '     Останов весового конвейера.';
                  1: sgAlarms1.Cells[2, CurrRaw] := sgAlarms1.Cells[2, CurrRaw] + '     Пуск весового конвейера.';
                  end;
                end;
              if NewMessage then
                begin
                  sgAlarms1.Cells[0, CurrRaw] := AlarmsQuery.Fields.FieldByName('dat').AsString;
                  Writeln(F, sgAlarms1.Cells[0, CurrRaw] + Chr(VK_TAB) + sgAlarms1.Cells[1, CurrRaw] + Chr(VK_TAB) + sgAlarms1.Cells[2, CurrRaw]);
                  CurrRaw := CurrRaw + 1;
                  sgAlarms1.RowCount := sgAlarms1.RowCount + 1;
                  NewMessage := False;
                end;
            end;
          i := i + 1;
  ...
  CloseFile(F);
  SendMessage(sgAlarms1.Handle, WM_SETREDRAW, 1, 0);
  sgAlarms1.Refresh;
  AlarmsQuery.Close;
  AlarmsQuery.Free;
  WStationForm.OPCReadTimer.Enabled := True;      //Включить обновление данных
...
procedure TDO1DO2Form.deAlarmsDate1Change(Sender: TObject);
begin
  SheetAlarms1Show(Sender);
end;

procedure TDO1DO2Form.clbAlarms1ClickCheck(Sender: TObject);
begin
  SheetAlarms1Show(Sender);
end;
