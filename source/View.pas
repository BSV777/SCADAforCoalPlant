...
type
  ...
  public
    procedure ViewTrends(Time1, Time2 : TDateTime; ObjList : TList);
...
procedure TViewForm.ViewTrends(Time1, Time2 : TDateTime; ObjList : TList);
var
  ObjEnum : byte;       //Перечислитель объектов
  SerieEnum : byte;     //Перечислитель серий
  Obj : TObject;
...
begin
  ...
  if Obj is TBatcher then
    begin
      ReportType := True;
      ReportChart.LeftAxis.Maximum := 200;
      ...
      SQLString := SQLString + ' from batchers2 where (dat > ''' +
    ...
    begin
      ReportType := False;
      ReportChart.LeftAxis.Maximum := 400;
      ...
      for ObjEnum := 0 to ViewingObjects.Count - 1 do
        begin
          ...
          SerieEnum := SerieEnum + 1;
          SQLString := 'select DAT, AS_TH, PR_TH from grade2_' + IntToStr(Grd.GradeNum) + ' where (dat > ''' +
            FormatDateTime('yyyy/mm/dd hh:nn:ss', BeginTime) + ''') and (dat < ''' +
            FormatDateTime('yyyy/mm/dd hh:nn:ss', EndTime) + ''');'; //SQLAnywhere
          ReportQuery.SQL.Add(SQLString);
          ReportQuery.Open;
