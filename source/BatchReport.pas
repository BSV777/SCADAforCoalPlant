procedure TDO1DO2Form.MakeBatchReport(BeginTime, EndTime : TDateTime);
var
...
//    RWorkBook := RExcel.WorkBooks.Add;
    ...
    for i := 1 to 16 do
      begin
        ...
        repeat
          GradeEndTime := Storage2.ViewStoredConfig(i, GradeBeginTime, EndTime, StoredBatchGrade);
          RRange := RWorkSheet.Cells[RecordsCounter + 3, 2];
          RRange.Value := Storage2.GradeNames[StoredBatchGrade];
          AssignBeginTime := GradeBeginTime;
            repeat
              AssignEndTime := Storage2.ViewStoredBatchers(i, AssignBeginTime, GradeEndTime, StoredBatchAssign);
              ...
              RRange.Value := Format('%8.1f', [Storage2.CalcBatchTime(i, AssignBeginTime, AssignEndTime) / 60]);
              RRange := RWorkSheet.Cells[RecordsCounter + 3, 6];
              RRange.Value := Format('%8.1f', [Storage2.CalcBatchSumm(i, AssignBeginTime, AssignEndTime)]);
